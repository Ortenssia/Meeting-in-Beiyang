...
class Helpers:
    ...
    def _detect_interfaces() -> List[dict]:
        ...
        if platform == 'android':
            try:
                ...
                for iface in Collections.list(ifaces):
                    ...
                    for addr in Collections.list(iface.getInterfaceAddresses()):
                        ip_addr = addr.getAddress()
                        if ip_addr and len(ip_addr.getAddress()) == 4:
                            ip = ip_addr.getHostAddress()
                            broadcast_obj = addr.getBroadcast()
                            broadcast = broadcast_obj.getHostAddress() if broadcast_obj else None
                            prefix_len = addr.getNetworkPrefixLength()
                            mask = Helpers._prefix_to_mask(prefix_len)
        ...
        # 2. 尝试 Unix/macOS/Linux 平台 (socket.getifaddrs)
        if not interfaces:
            try:
                import socket
                if hasattr(socket, "getifaddrs"):
                    for info in socket.getifaddrs():
                        name, family, flags, sockaddr, netmask, broadcast = info
                        if family == socket.AF_INET and sockaddr:
                            ip = sockaddr[0]
                            if ip.startswith("127."):
                                continue
                            mask = netmask[0] if netmask else "255.255.255.0"
                            bcast = broadcast[0] if broadcast else None
                            interfaces.append({
                                "name": name,
                                "ip": ip,
                                "mask": mask,
